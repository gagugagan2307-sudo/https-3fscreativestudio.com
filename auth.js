...
(function(){
  ...
  function mount(){
    ...
    document.getElementById('threefs-login-form').addEventListener('submit',function(e){e.preventDefault();const a=document.getElementById('threefs-admin').value.trim(),p=document.getElementById('threefs-pass').value;if(a===ADMIN&&p===PASS){loggedIn=true;localStorage.setItem(KEY,'1');wrap.remove();document.body.classList.remove('threefs-locked');emit();}else document.getElementById('threefs-login-error').textContent='Incorrect admin or password.';});
  }
